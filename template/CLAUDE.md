...
- **PAT expired or insufficient scope (401, 403):** re-bootstrap step 3b. Most common cause of session-start failure.
- **Connection error on a `curl` to `api.github.com`:** Domain Allow List doesn't permit it (or the change hasn't propagated to this chat). Re-check Settings per BOOTSTRAP step 5; if the change was made in this same chat session, the user may need to start a fresh chat to pick up the new allow-list.
