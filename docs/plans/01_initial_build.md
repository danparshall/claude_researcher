...
**Status (as of 2026-05-09 evening):** Phases 1–4 complete; Phase 4 smoke-tested end-to-end with Step 9 validation passing in a fresh claude.ai chat. Phase 4.5 (collaborator mode) deferred to v1.1. Phase 4.6 (RESEARCHER.md retrofit from AITaxBID audit; originally spec'd against `CLAUDE.md`, now lands into the renamed file) and Phase 6 expansions for AITaxBID-sourced skill ports added to this plan during the same session that ran the smoke test. See [`STATUS.md`](../../STATUS.md) for current state and the [`docs/convos/`](../convos/) directory for session records, especially:
...
| **4.6** RESEARCHER.md retrofit from AITaxBID audit | Five Tier B/C patterns universalized in `template/RESEARCHER.md` (originally targeted `template/CLAUDE.md`, redirected to the renamed file by Plan 03) | ✅ **Done** (2026-05-11, commit `86dfeb3`, ~45 min). Shipped as single commit alongside Plan 03 execution. Source: AITaxBID audit (PR #1, merged `6397c33`). §1.5 Resumption discipline, §5 Working conventions block (don't-infer-ask + show-before-committing + codify-after-three), §8 Parking Lot. | None — Phase 4.6 complete |
...
| **10** Publish | Decide strategy, update README, test from clean machine, open placeholder issues | 🟡 **Mostly done.** Task 58 (publish strategy): resolved — dev-repo-public confirmed deliberate; root README has an "About" section framing the project as a Dan + Andrea collaboration. Task 59 (README bootstrap prompt): done — moved to ROOT README, commit `2b8d03d`. Task 60 (test from clean machine): done via 2026-05-09 incognito chats. | Task 61 (placeholder issue to validate pre-filled URL composition): ~15 min. |
...
4. **Phase 4.6 RESEARCHER.md retrofit** — small, focused. ~30-60 min.
...
26.5.4. **`Role: owner | collaborator` field in `personal_info.md` schema.** New field, set during bootstrap interview. RESEARCHER.md uses it to choose between owner-mode wrap-up (PR-and-self-merge) and collaborator-mode wrap-up (PR-only-stop).

26.5.5. **Wrap-up flow split.** RESEARCHER.md §6 already handles the merge-blocked case gracefully; v1.1 just makes the collaborator path a first-class flow rather than a fallback. The directory move (`docs/active/<X>/` → `docs/historical/<X>/`) lives on the owner's plate, triggered when the owner next sees the merged PR in their session.
...
## Phase 4.6 — `template/RESEARCHER.md` retrofit from AITaxBID audit

The Phase 4 production runtime instructions file (committed `681ed9d` as `template/CLAUDE.md`, restructured through `42648a4`, and renamed to `template/RESEARCHER.md` by Plan 03) was written before the AITaxBID skills audit landed (PR #1, merged `6397c33`). The audit identified five Tier B/C patterns worth promoting from per-step practice to universal rules in RESEARCHER.md. Each is currently *partially* covered (lives as per-confirmation-gate or per-step practice); the retrofit makes each one a universal rule. Source: [`docs/convos/20260509_aitaxbid_skills_audit.md`](../convos/20260509_aitaxbid_skills_audit.md) and the synthesis convo [`docs/convos/20260509_phase6_skill_port_planning.md`](../convos/20260509_phase6_skill_port_planning.md).

26.6.1. **Tracker-not-past-chats discipline.** Add to `template/RESEARCHER.md` as new §1.5 (or top of §2). Verbatim source from Andrea's `writing_skill.md`: *"Do NOT search past chats or conversations. The tracker is the single source of truth for where we left off."* claude.ai-specific because past chats are temptingly available but unreliable. Promote to a top-of-file rule: "Resumption mechanism is STATUS.md + RESEARCH_LOG.md, NOT past chats. If a user references 'last time we said X,' verify against the trackers; do not trust chat memory." *Note: the 2026-05-11 dogfooding session shipped a partial version of this rule as the "Catch-up source" paragraph in §2e — verify what's already in RESEARCHER.md before adding.*

26.6.2. **3+-repetition codification rule.** Add to `template/RESEARCHER.md` as a new working-conventions block under §5. Verbatim source from Andrea's `CLAUDE_TEMPLATE.md`: *"If the user asks for the same type of task 3+ times, check whether it should be codified as a rule in this file."* Sharp threshold avoids both premature and too-late codification.

26.6.3. **Parking Lot section.** Add as new §8 (or Appendix) in `template/RESEARCHER.md`. Source: Andrea's `CLAUDE_TEMPLATE.md` ships a "Parking Lot (To Be Defined)" section for undecided items; items move out as decided. Currently the runtime RESEARCHER.md tracks open questions only via the plan file and convos — adding a Parking Lot to RESEARCHER.md gives the runtime instruction set its own home for self-tracked open questions (e.g., the `raw.githubusercontent.com` allow-list finding from the smoke test).
...
26.6.5. **Show-before-committing universalized.** Add to the new working-conventions block under §5. Existing RESEARCHER.md scripts gates at sensitive boundaries (§3 branch creation, §5 destructive deletes, §6 archive/merge); audit recommends promoting to a universal rule applied to *all* writes. Add: "Before any write to the user's repos, briefly state what you're about to write and why. The user can interject before the commit lands." Reframes per-gate confirmations as the default rather than exceptions.
...
26.6.7. **Commit:** `Phase 4.6: RESEARCHER.md retrofit — universalize tracker discipline, codification, parking lot, do-not-infer, show-before-committing`.
...
- Issue-filing pre-filled URL: URL-encode title and body; include git_fluency tier + RESEARCHER.md SHA in body for triage; never include PAT or personal_info contents beyond the tier.
...
- **What happens when Dan's RESEARCHER.md upstream changes mid-session for a user?** A user mid-session won't re-fetch RESEARCHER.md unless explicitly prompted (e.g., `git pull` in the clone directory; see Plan 03). Acceptable; no action needed unless we hit a real-world bug.
...
- **Per-user-repo `CLAUDE_TEMPLATE.md`.** Andrea's pattern of a per-project CLAUDE.md with `Generic skill: ... project-specific implementation` blocks is portable in form, but claude_researcher's design currently has STATUS.md + personal_info.md + the upstream runtime RESEARCHER.md serving part of that role. Whether each user's research repo would benefit from its own short project-specific CLAUDE.md (overriding or extending upstream) is a real design question. The Plan 03 rename (`template/CLAUDE.md` → `template/RESEARCHER.md`) frees the `CLAUDE.md` name for exactly this use; tracked here, still deferred until Phase 6 ports demonstrate (or fail to demonstrate) the need.
